...
which are thus available to be adapted. It also implements the _Target Interface_, in order to be compatible with what
...
[WARNING]
====
The consequence of having direct access to `ConcreteAdaptee` method in the `Adapter` class and the fact that the class
